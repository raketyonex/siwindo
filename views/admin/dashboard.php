    ...
    <div class="container mt-4">
        ...
        <form method="get" class="mb-4">
            ...
            <select class="form-control" id="jenis" name="jenis" onchange="this.form.submit()">
                ...
                <option value="gunung" <?= isset($_GET['jenis']) && $_GET['jenis'] == 'gunung' ? 'selected' : ''; ?>>Gunung</option>
                <option value="pantai" <?= isset($_GET['jenis']) && $_GET['jenis'] == 'pantai' ? 'selected' : ''; ?>>Pantai</option>
            </select>
        </form>
                    ...
                    <?php foreach ($wisata as $key => $item): ?>
                        <?php if (empty($_GET['jenis']) || $_GET['jenis'] == $item['jenis']): ?>
                            <tr>
                                ...
                                <td>
                                    <a href="<?= base_url('admin/edit_wisata/'.$item['id']); ?>" class="btn btn-warning btn-sm">Edit</a>
                                    <a href="<?= base_url('admin/hapus_wisata/'.$item['id']); ?>" class="btn btn-danger btn-sm" onclick="return confirm('Apakah Anda yakin?')">Hapus</a>
                                </td>
                            </tr>
                        <?php endif; ?>
                    <?php endforeach; ?>
